...
mod emission_tests {
    ...
    /// Phase 63 W3 made both spellings emit identical bytes so that a file
    /// could be migrated by re-emitting it. Phase 70 finished the migration:
    /// the `_ms` spelling no longer parses, and what a contract carries is
    /// always the canonical form, so what is emitted is too.
    #[test]
    fn the_canonical_spelling_is_what_is_emitted() {
        ...
        assert!(!out.contains("max_latency_ms"), "{out}");
    }
}
