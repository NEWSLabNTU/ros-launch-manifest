...
#### Cross-repo design agreement (2026-07-19) — SSoT structure, per-platform realization

Reconciles this scheduling track (Scheduling-SSoT, phase-45) with nano-ros's RTOS mapper (nano-ros RFC-0050/RFC-0052). **`play_launch resolve` is the single source of truth for scheduling *structure*: it runs the chain mapper once and embeds the resolved chain/graph structure in the model's `execution:` layer** (FQN-qualified chains — `via` topics + segment/boundary decomposition — plus per-(node, path) requirement facts: trigger, deadline, budget, criticality). Both back-ends read this one structure; the DAG is resolved once, never re-derived per consumer.

- **Structure is shared; *realization* is per-platform.** play_launch realizes the structure as Linux fixed-priority (PiCAS `ChainAwareDetail` ranks). nano-ros runs its **own** RTOS-framework-aware mapper (RFC-0052) that binds the same structure to kernel features (EDF / preemption-threshold / sporadic-reservation / affinity) via a six-dim agnostic requirement (`activation, urgency, deadline, budget, non_preempt_scope, placement`), kernel-native preferred, executor backfill (EDF/sporadic/LET) where a kernel lacks it, per-platform guarantee differences **recorded**. nano-ros does **not** consume the per-path ranks — those stay Linux-side (keeping them in the model is harmless; `provenance` may surface for diagnostics).
- **Runtime E2E monitoring stays stamp-based — no chain-id.** The graph in the model is a *bake-time* input to the mappers. At run time, freshness is `age = now − header.stamp` at the sink (`sub_endpoints.max_age_ms`), per the stamp-preservation convention in `launch-manifest.md` §Timestamps. The subscription topic disambiguates the budget; the message carries its own origin time.
- **Type sharing (45.3):** the resolved chain/trigger structs (`types::{Trigger,EffectiveTrigger,ChainDecl,…}`, `sched::{ResolvedChain,ChainElement,MapperPath,ChainAwareDetail}`) are shared into the `model` crate — no third hand-mirror (the "one schema, no hand-mirroring" rule, issue 0160).

Cross-refs nano-ros RFC-0050 §"Shared input + SSoT structure, per-platform realization" and RFC-0052 §"nano-ros answer + realization design."
