...
- **Causality + execution modeling is the consumer's job; the *algorithm* is shared, not the output.** The chain/DAG resolution, scheduling, and task creation are done by each **runtime**: play_launch's *Linux runtime* (chain_aware/PiCAS) and nano-ros's RTOS mapper (RFC-0052). The reusable value — DAG/causality/segment + chain-mapper — is **extracted into standalone reusable crate(s)** (decoupled from this `sched`/`model` layer) that both runtimes call; each applies its own realizer. One algorithm, two realizations, no embedded output.
...
**Rework:** (1) revert `model.execution.sched`/`ExecutionSched` + `sched`-struct re-exports in `model` (`78f637d`/`5016b4d`/`c47c5bd`); (2) extract the DAG/causality/segment + chain-mapper algorithm into standalone reusable crate(s). Cross-refs nano-ros RFC-0050 §"Input model; causality + execution modeling per consumer" and RFC-0052 §"nano-ros execution modeling."
